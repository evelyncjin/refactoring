...
public class StatementPrinter {
    ...
    // Needed to satisfy VisibilityModifier
    ...
    public String statement() {
        ...
        for (final Performance p : invoice.getPerformances()) {
            ...
            switch (play.getType()) {
                case "tragedy":
                    ...
                    if (p.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                        thisAmount += EXTRA_TRAGEDY_AMOUNT
                                * (p.getAudience() - TRAGEDY_BASE_AUDIENCE);
                    }
                    break;
            ...
            result.append(
                    String.format(
                            "  %s: %s (%s seats)%n",
                            play.getName(),
                            frmt.format(thisAmount / CENT_DIVISOR),
                            p.getAudience()
                    )
            );
        ...
        result.append(
                String.format(
                        "Amount owed is %s%n",
                        frmt.format(totalAmount / CENT_DIVISOR)
                )
        );
